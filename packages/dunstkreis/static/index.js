...
async function main() {
    ...
    function syncTime() {
        ...
        // The same instant in the viewer's own zone. Everything the library sees is UTC, since a Julian Day
        // has no notion of one, but UTC is not what anyone reads a sunset off, so both are shown.
        const utc = Date.UTC(t.year, t.month - 1, Math.floor(t.day), t.hour, t.minute, Math.floor(t.second));
        $("localTime").textContent = new Date(utc).toLocaleString(undefined, {
            year: "numeric",
            month: "2-digit",
            day: "2-digit",
            hour: "2-digit",
    ...
    $("fullscreen").addEventListener("click", () => {
        ...
        else $("sky").requestFullscreen?.();
    });
    // Entering or leaving fullscreen changes the canvas's CSS size, which render() reads to size its backing
    // store, so it has to re-run. The browser fires this after the layout has settled.
    document.addEventListener("fullscreenchange", () => {
        $("fullscreen").textContent = document.fullscreenElement ? "exit fullscreen" : "fullscreen";
        requestRender();
    });

    // Tracks the canvas's own box rather than the window's, so the backing store follows it through a
    // fullscreen transition, a devicePixelRatio change or a first paint that lands before layout settles.
